document.addEventListener("DOMContentLoaded", () => {
  ...
  const rosePetalLayer = document.getElementById("rosePetalLayer");
  const ashirwadButton = document.getElementById("ashirwadButton");
  const ashirwadMessage = document.getElementById("ashirwadMessage");

  function showerRosePetals(count = 38){
    if(!rosePetalLayer) return;
    for(let i=0;i<count;i++){
      const petal = document.createElement("span");
      petal.className = "rose-petal" + (i % 3 === 0 ? " soft" : "");
      petal.style.left = `${Math.random()*100}%`;
      petal.style.setProperty("--fall-time", `${4.4 + Math.random()*3.4}s`);
      petal.style.setProperty("--drift", `${-80 + Math.random()*160}px`);
      petal.style.animationDelay = `${Math.random()*1.25}s`;
      rosePetalLayer.appendChild(petal);
      setTimeout(() => petal.remove(), 9000);
    }
  }

  /* A few petals after the invitation opens, then a full blessing shower on demand. */
  document.getElementById("openInvitation")?.addEventListener("click", () => {
    setTimeout(() => showerRosePetals(10), 850);
  });

  ashirwadButton?.addEventListener("click", () => {
    showerRosePetals(46);
    ashirwadMessage?.classList.add("show");
    ashirwadButton.textContent = "🙏 बाप्पाचा आशीर्वाद सदैव राहो 🙏";
  });
